...
class GraphDBBase():
    def __init__(self, command=None, argv=None, extended_options='', extended_long_options=[]):
        ...
        self.neo4j_params = config['neo4j']

        uri = self.uri or os.getenv('NEO4J_URI') or self.neo4j_params.get('uri', 'bolt://localhost:7687')
        user = self.neo4j_user or os.getenv('NEO4J_USER') or self.neo4j_params.get('user', 'neo4j')
        password = self.neo4j_password or os.getenv('NEO4J_PASSWORD') or self.neo4j_params.get('password', 'password')
        ...
        other_params = dict([(key, maybe_convert(key, value)) for key, value in self.neo4j_params.items()
    ...
    def get_database(self):
        return self.database or os.getenv('NEO4J_DATABASE') or self.neo4j_params.get('database', 'neo4j')
